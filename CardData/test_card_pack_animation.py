...
class TestCardPackAnimation(unittest.TestCase):
    def test_render(self):
        ...
        with self.subTest(msg="holo/stars in perfect/holo frame"):
            print(frames[10])
            self.assertIn("\x1b[34m*****\x1b[0m", frames[10])
        ...
        with self.subTest(msg="snapshot looks correct"):
            ...
            self.assertIn("┃<artline>┃ │<artline>│ │<artline>│ │<artline>│\n", frames[10])
            self.assertIn("┃<start01>┃ │<start02>│ │<start03>│ │<start04>│\n", frames[10])
            self.assertIn("┃<end  01>┃ │<end  02>│ │<end  03>│ │<end  04>│\n", frames[10])
            self.assertIn("┗━━\x1b[32m*****\x1b[0m━━┛ └─────────┘ └─────────┘ └─────────┘\n", frames[10])
            self.assertIn("\n", frames[10])
            self.assertIn("┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐\n", frames[10])
            self.assertIn("│<start05>│ │<start06>│ │<start07>│ │<start08>│\n", frames[10])
            self.assertIn("│<end  05>│ │<end  06>│ │<end  07>│ │<end  08>│\n", frames[10])
            self.assertIn("└─────────┘ └─────────┘ └─────────┘ └─────────┘\n", frames[10])
            self.assertIn("            ┌─────────┐ ┏━━\x1b[34m*****\x1b[0m━━┓\n", frames[10])
            self.assertIn("            │<start09>│ ┃<start10>┃\n", frames[10])
            self.assertIn("            │<artline>│ ┃<artline>┃\n", frames[10])
            self.assertIn("            │<end  09>│ ┃<end  10>┃\n", frames[10])
            self.assertIn("            └─────────┘ ┗━━\x1b[34m*****\x1b[0m━━┛\n", frames[10])
