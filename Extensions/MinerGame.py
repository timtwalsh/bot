...
class MinerGame(commands.Cog):
    ...
    async def load_data(self):
        """Load mining game data from file"""
        ...
        try:
            with open(f'/app/data/{self.qualified_name}_data.json', 'r+') as in_file:
                ...
                # Initialize power tracking dictionaries if they don't exist
    ...
    async def mybase(self, ctx, member: discord.Member = None):
        ...
        if member_id not in self.member_generators:
            self.init_member(member_id)
        
        print(self.member_total_power_usage)
        print(self.member_total_power)
        print(member.id)
        
        msg = f"{member.name}'s Base - Last Power Bill - Supply: `{self.member_total_power[member_id]:.2f}kW-h` Demand: `{self.member_total_power_usage[member_id]:.2f}kW-h`\n```"
        user_generators = self.member_generators[member_id]
        user_miners = self.member_miners[member_id]
    ...
    @commands.command(name="power")  # FIXED: Added missing command decorator
    async def power(self, ctx, *, member: discord.Member = None):
        ...
        power_supply = self.member_total_power[member_id]
        print('power_supply', power_supply)
        power_usage = self.member_total_power_usage[member_id]
    ...
    async def buy_miner(self, user_id="", item_name=""):
        ...
        # Ensure member is initialized
        if user_id not in self.member_miners:
            self.init_member(user_id)
    ...
    async def buy_generator(self, user_id="", item_name=""):
        ...
        # Ensure member is initialized
        if user_id not in self.member_generators:
            self.init_member(user_id)
    ...
    async def buy(self, ctx, item_name: str = ""):
        ...
        await ctx.send(outcome, delete_after=self.bot.MEDIUM_DELETE_DELAY)
        await ctx.message.delete(delay=self.bot.SHORT_DELETE_DELAY)
    ...
    async def save_data(self):
        if len(self.member_generators) > 0:
            save_data = {'uptime': self.time_elapsed,
                         ...
                         'member_total_power': self.member_total_power,  # FIXED: Save these too
    ...
    async def timeout(self):
        if self.time_elapsed >= SMALLEST_PAYMENT_TIMER:
            ...
            if self.world_power_supply > 0:  # FIXED: Prevent division by zero
