...
class MagicTheShekelling(commands.Cog):
    ...
    async def load_data(self):
        ...
        try:
            with open(f'/app/data/{self.qualified_name}_collections.json', 'r') as f:
                data = json.load(f)
                self.game.user_collections = defaultdict(lambda: defaultdict(int), data.get('collections', {}))
                self.game.user_enhancements = defaultdict(list, data.get('enhancements', {}))
                print(f"Loaded {len(data.get('collections', {}))} user collections and {len(data.get('enhancements', {}))} enhancement profiles for Magic the Shekelling.")
    ...
    async def save_data(self):
        ...
        try:
            save_data = {
                'collections': dict(self.game.user_collections),
                'enhancements': dict(self.game.user_enhancements)
            }
