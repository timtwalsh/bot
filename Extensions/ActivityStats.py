...
class ActivityStats(commands.Cog):
    def __init__(self, bot):
        ...
        self.ignored_activities = ["Spiralling Ever Downwards"]
    ...
    async def load_data(self):
        try:
            ...
            print(f"NOT FOUND {self.qualified_name}_data.json")
            for member in self.bot.get_all_members():
                role = discord.utils.get(member.guild.roles, name="game")
                if role in member.roles:
                    for activity in member.activities:
                        if activity is not None and str(activity.type) == "ActivityType.playing" and activity.name not in self.ignored_activities:
                            self.activities[activity.name] = 0
                            self.user_activities[str(member.id)] = {}
                            self.user_activities[str(member.id)][activity.name] = 0
                        ...
                        if activity is not None and str(activity.type) == "ActivityType.playing" and activity.name not in self.ignored_activities:
                            ...
                            try:
                                ...
                                if current_activity not in self.user_activities[str(member.id)].keys():
                                    print("New User Activity:", activity.name, end=", ")
                                    self.user_activities[str(member.id)][current_activity] = self.bot.TICK_RATE
                                else:
                                    self.user_activities[str(member.id)][current_activity] += self.bot.TICK_RATE
                                    # if activity.name == "BlueStacks":
                                    #     print(activity.type,activity.name,activity.state,activity.details)
                            except Exception:
                                print("ERROR:", member)
                    # if DEBUG:
